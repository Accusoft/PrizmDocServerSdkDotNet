...
namespace Accusoft.PrizmDocServer.Tests
{
  public static class FileAssert
  {
    ...
    {
      CollectionAssert.AreEqual(ASCIIEncoding.ASCII.GetBytes("%PDF-"), FileUtil.ReadFileHeader(filename, 5), "File header bytes did not indicate PDF.");
    }

    public static void IsTiff(string filename)
    {
      var header = FileUtil.ReadFileHeader(filename, 4);

      // See https://www.adobe.io/content/dam/udp/en/open/standards/tiff/TIFF6.pdf
      if (header[0] == (byte)'I') { // Intel (II), little-endian.
        CollectionAssert.AreEqual(new byte[] {(byte)'I', (byte)'I', 42, 0}, header, "File header bytes did not indicate TIFF.");
      } else { // Motorola (MM), big-endian.
    ...
    {
      var header = FileUtil.ReadFileHeader(filename, 8);
    ...
    {
      var header = FileUtil.ReadFileHeader(filename, 3);
