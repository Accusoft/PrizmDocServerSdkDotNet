...
namespace Accusoft.PrizmDocServer.Exceptions
{
  internal class ErrorData
  {
    ...
    internal static async Task<ErrorData> From(HttpResponseMessage response)
    {
      ...
      return ErrorData.From(response.StatusCode, response.ReasonPhrase, body);
    }
    ...
    internal static ErrorData From(HttpStatusCode statusCode, string reasonPhrase, string body)
    {
      ...
      try { rawErrorDetails = JObject.Parse(body)["errorDetails"].ToString(); } catch { }

      var innerErrors = new List<InnerErrorData>();

      JArray results = null;
      try { results = ((JArray)JObject.Parse(body)["output"]["results"]); } catch { }
      ...
      if (results != null)
      {
        foreach (var result in results)
        {
          ...
          try { innerErrorCode = (string)result["errorCode"]; } catch { }

          if (innerErrorCode == null)
            continue;

          string rawInnerErrorDetails = null;
          try { rawInnerErrorDetails = result["errorDetails"].ToString(); } catch { }
      ...
      var reasonPhraseAndOrErrorCode = string.Join(" ", new [] { reasonPhrase, errorCode }.Distinct()).Trim();
